...
use App\Services\MyBank\Account\GetBalanceService as AccountGetBalanceService;
use App\Services\MyBank\Financial\GetBalanceService;
...
class UserController extends Controller
{
    ...
    public function dashboard(AccountGetBalanceService $getBalanceService)
{
    $transactions = Transaction::where('user_id', Auth::id())
        ->orderBy('created_at', 'DESC')
        ->get();

    $balanceData = $getBalanceService->GetRealBalance([]);

    // Loga para debug
    \Log::info('Saldo retornado do serviço:', $balanceData);

    // Trata os valores para reais (dividido por 100)
    $balanceAvailable = isset($balanceData['balanceAvailable']) ? $balanceData['balanceAvailable'] / 100 : 0;
    $toReceive = isset($balanceData['toReceive']) ? $balanceData['toReceive'] / 100 : 0;
    ...
    {
        $user = User::where('pix_key', $pixKey)->first();
        return response()->json(['user' => $user]);
    }


    //Edit Profile and Password Manager
    public function ManagerProfile()
    {
        $id = Auth::user()->id;
        $managerData = User::find($id);
        return view('agency.manager.profile', compact('managerData'));
    }
    ...
    public function ManagerProfileStore(Request $request)
    {
        ...
        if ($request->file('avatar')) {
            ...
            @unlink(public_path('uploads/manager_images/' . $data->avatar));
            $filename = date('YmdHi') . $file->getClientOriginalName();
            $file->move(public_path('uploads/manager_images'), $filename);
    ...
    {
        return view('agency.manager.pass');
    }
    ...
    //Edit Profile and Password User
    public function UserProfile()
    {
        $id = Auth::user()->id;
        $userData = User::find($id);
        return view('users.profile.profile', compact('userData'));
    }
    ...
    public function UserProfileStore(Request $request)
    {
        ...
        if ($request->file('avatar')) {
            $file = $request->file('avatar');
            @unlink(public_path('uploads/user_images/' . $data->avatar));
            $filename = date('YmdHi') . $file->getClientOriginalName();
    ...
    {
        // Obter o endereço IP da máquina logada
        $ip = $request->ip();
    ...
    public function validateEmail($email)
    {
        ...
        // Busque a agência associada ao usuário, se existir
        $agency = $user->agency; // Certifique-se de que o relacionamento está definido no modelo User
        if (!empty($agency)) {
            $agency = Agency::find(1);
        }
        // Passe o usuário e a agência para a view
        return view('auth.validate', compact('user', 'agency'));
    }
    ...
    public function checkEmail(Request $request)
    {
        ...
        if ($user) {
            // Se o usuário existir, redireciona para a página de atualização com os dados do usuário
            return redirect()->route('view.validate.email', ['email' => $user->email]);
        } else {
            // Se o usuário não existir, redireciona para a página de criação de conta
    ...
    public function updateUser(Request $request)
    {
        ...
        if (!$user) {
            ...
            $notification = ['message' => 'Nenhuma conta foi encontrada', 'alert-type' => 'error'];
            return redirect()->back()->with($notification);
        }
        ...
        $request->validate([
            ...
            'email' => [
                ...
                'email',
                'max:255',
                // A regra 'unique' ignora o registro atual baseado no ID do usuário
                Rule::unique('users', 'email')->ignore($user->id),
                function ($attribute, $value, $fail) use ($user) {
                    $exists = User::where('email', $value)
                        ->where('id', '!=', $user->id) // Ignora o usuário atual
                        ->exists();
                    if ($exists) {
        ...
        function saveBase64Image($base64Image, $folderPath, $fileName)
        {
        ...
        // Salvar imagens de base64 em arquivos
        $documentFrontImageName = saveBase64Image($request->document_front, 'uploads/userdocs/documents/', 'document_front');
        $documentBackImageName = saveBase64Image($request->document_back, 'uploads/userdocs/documents/', 'document_back');
        $selfieImageName = saveBase64Image($request->selfie, 'uploads/userdocs/documents/', 'selfie');

        // Atualizar as colunas do usuário com os caminhos das imagens
        $user->document_front = '/uploads/userdocs/documents/' . $documentFrontImageName;
        $user->document_back = '/uploads/userdocs/documents/' . $documentBackImageName;
        $user->selfie = '/uploads/userdocs/documents/' . $selfieImageName;

        //Função para gerar um número de cartão de único
    ...
    RAL Consultoria Serviços Financeiros Investimentos e Aplicações ltda. E LIBERTY BANK
        ...
        return redirect('/checkout')->with('success', 'Dados atualizados com sucesso!');
    }
}
