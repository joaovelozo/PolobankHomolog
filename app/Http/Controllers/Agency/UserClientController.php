...
class UserClientController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $polocal = Split::where('title', 'like', '%Polocal%')->first();
            if ($polocal) {
                UserServices::create([
                    'status' => 'active',
                    'user_id' => $user->id,
                    'payment_id' => $polocal->id,
                ]);
            }
        ...
        Assinatura do Gestor de Negócios_____________________________
            ...
            return  redirect()->back()->withInput()->withErrors(['message' => 'Erro ao criar contrato. Por favor, tente novamente.']);
        }
    }
    ...
    {
        $user = User::findOrFail($id);
        return view('agency.user.edit', compact('user'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        return redirect()->route('clientusers.index')->with($notification);
    }
