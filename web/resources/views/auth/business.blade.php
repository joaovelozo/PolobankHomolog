...
<body>
        ...
        <section class="pt-16 pb-32">
            <div class="container">
                <div class="mw-md mx-auto text-center">
                    ...
                    <br>
                    <br>
                    <h3 class="mb-4">Crie Sua Conta PJ</h3>
                    ...
                    <form method="POST" id="documentForm" action="{{ route('business.register') }}"
                        ...
                        <input type="hidden" name="cardBusiness" id="cardBusiness">
                        <input type="hidden" name="imagecontrato" id="imagecontrato">
                        <input type="hidden" name="imagecomprovante_endereco" id="imagecomprovante_endereco">


                        <h5 class="mb-5">Dados da Empresa</h5>

                        <div class="mb-5">
                            <input class="form-control" type="text" name="documentBusiness" id="cpfCnpj"
                                value="{{ old('documentBusiness') }}" placeholder="Digite O CNPJ" required>
                        </div>
                        <div class="mb-5">
                            <input class="form-control" type="text" name="companyName"
                                value="{{ old('companyName') }}" placeholder="Nome da Empresa" required>
                        </div>
                        <div class="mb-5">
                            <input class="form-control" type="text" name="fantasyName"
                                value="{{ old('fantasyName') }}" placeholder="Nome Fantasia" required>
                        </div>
                        <div class="mb-5">
                            <input class="form-control" type="text" name="cnaeCode" value="{{ old('cnaeCode') }}"
                                placeholder="Digite O CNAE Principal" required>
                        </div>
                        <div class="mb-5">
                            <input class="form-control" type="text" placeholder="Inscrição Estadual"
                                value="{{ old('stateRegistration') }}" name="stateRegistration">
                        </div>
                        <div class="mb-5">
                            <select class="form-control" name="simpleNational" required>
                                <option value="" disabled
                                    {{ old('simpleNational') === null ? 'selected' : '' }}>
                                    Tipo de Empresa</option>
                                <option value="MEI" {{ old('simpleNational') === 'MEI' ? 'selected' : '' }}>
                                    MEI
                                </option>
                                <option value="EIRELI" {{ old('simpleNational') === 'EIRELI' ? 'selected' : '' }}>
                                    EIRELI
                                </option>
                                <option value="LTDA" {{ old('simpleNational') === 'LTDA' ? 'selected' : '' }}>LTDA
                                </option>
                                <option value="S/A" {{ old('simpleNational') === 'S/A' ? 'selected' : '' }}>S/A
                                </option>
                                <option value="EI" {{ old('simpleNational') === 'EI' ? 'selected' : '' }}>EI
                                </option>
                                <option value="SS" {{ old('simpleNational') === 'SS' ? 'selected' : '' }}>SS
                                </option>
                                <option value="SLU" {{ old('simpleNational') === 'SLU' ? 'selected' : '' }}>SLU
                                </option>
                        ...
                        <div class="mb-5">
                            <label style="display: block; text-align: left;"><b>Data de Abertura da Empresa</b></label>
                            <input class="form-control" type="date" name="openDate"
                                value="{{ old('openDate') }}" required>
                        </div>
                        <div class="mb-5">
                            <input class="form-control" type="text" placeholder="Faturamento"
                                value="{{ old('companyRevenue') }}" name="companyRevenue" id="income">
                        </div>
                        <h5>Endereço da Empresa</h5>

                        <div class="mb-5">
                            <input class="form-control" id="cep1" name="businessZipCode"
                                onblur="buscarEndereco(1)" placeholder="CEP" required
                                value="{{ old('businessZipCode') }}">
                        </div>

                        <div class="mb-5">
                            <input class="form-control" id="endereco1" name="businnesAddress"
                                value="{{ old('businnesAddress') }}" required placeholder="Endereço" readonly>
                        </div>

                        <div class="mb-5">
                            <input class="form-control" type="text" name="businessAddressNumber"
                                placeholder="Número" value="{{ old('businessAddressNumber') }}">
                        </div>

                        <div class="mb-5">
                            <input class="form-control" id="bairro1" name="businessNeighborhood"
                                value="{{ old('businessNeighborhood') }}" required placeholder="Bairro" readonly>
                        </div>

                        <div class="mb-5">
                            <input class="form-control" id="cidade1" name="businessCity" placeholder="Cidade"
                                value="{{ old('businessCity') }}" readonly required>
                        </div>

                        <div class="mb-5">
                            <input class="form-control" id="estado1" name="businessState" placeholder="Estado"
                                value="{{ old('businessState') }}" readonly required>
                        </div>
                        <h5>Upload de Documentos</h5>

                        <div class="mb-5">
                            <label>Cartão CNPJ</label>
                            <input id="fileCardBusiness" type="file" accept=".pdf" required>
                        </div>

                        <div class="mb-5">
                            <label>Contrato Social</label>
                            <input id="fileImageContrato" type="file" accept=".pdf" required>
                        </div>

                        <div class="mb-5">
                            <label>Comprovante de Endereço da Empresa</label>
                            <input id="fileComprovanteEndereco" type="file" accept="image/*" required>
                        </div>

                        <h5 class="mb-5">Informação do Sócio</h5>
                        <div class="mb-5">
                            <input class="form-control" type="text" name="name"
                                value="{{ old('complement') }}" placeholder="Digite Seu Nome">
                        </div>
                        <div class="mb-5">
                        ...
                        <div class="mb-5">
                            <input class="form-control" type="text" name="username"
                                value="{{ old('username') }}" placeholder="Como Gostaria de Ser Chamado?" required>
                        </div>
                        <div class="mb-5">
                        ...
                        <div class="mb-5">
                            ...
                            <input class="form-control" type="date" id="issueDate"
                                value="{{ old('issueDate') }}" name="issueDate" placeholder="Data de Emissão"
                                required>
                        </div>
                        <div class="mb-5">
                        ...
                        <h5>Dados Residenciais do Sócio</h5>

                        <div class="mb-5">
                            <input class="form-control" id="cep2" name="zipCode" onblur="buscarEndereco(2)"
                                placeholder="CEP" required value="{{ old('zipCode') }}">
                        </div>

                        <div class="mb-5">
                            <input class="form-control" id="endereco2" name="address" value="{{ old('address') }}"
                                required placeholder="Endereço" readonly>
                        </div>
                        ...
                        <div class="mb-5">
                            <input class="form-control" id="bairro2" name="neighborhood"
                                value="{{ old('neighborhood') }}" required placeholder="Bairro" readonly>
                        </div>

                        <div class="mb-5">
                            <input class="form-control" id="cidade2" name="city" placeholder="Cidade"
                                value="{{ old('city') }}" readonly required>
                        </div>

                        <div class="mb-5">
                            <input class="form-control" id="estado2" name="state" placeholder="Estado"
                        ...
                        <div class="mb-5">
                            ...
                            <input id="fileImagedoc" type="file" accept="image/*" required>
                        </div>

                        <div class="mb-5">
                            <label>Verso do Documento</label>
                            <input id="fileImagedocVerso" type="file" accept="image/*" required>
                        </div>

                        <div class="mb-5">
                            <label>Comprovante de Endereço</label>
                            <input id="fileComprovante" type="file" accept="image/*" required>
                        </div>

                        <div class="mb-5">
                            <label>Selfie com o Documento</label>
                            <input id="fileSelfie" type="file" accept="image/*" required>
                        </div>
    ...
    <script>
        function buscarEndereco(id) {
            const cep = document.getElementById(`cep${id}`).value.replace(/\D/g, '');
            if (cep !== '') {
                ...
                if (validacep.test(cep)) {
                    fetch(`https://viacep.com.br/ws/${cep}/json/`)
                        ...
                        .then(data => {
                            if (!data.erro) {
                                document.getElementById(`endereco${id}`).value = data.logradouro;
                                document.getElementById(`bairro${id}`).value = data.bairro;
                                document.getElementById(`cidade${id}`).value = data.localidade;
                                document.getElementById(`estado${id}`).value = data.uf;
                            } else {
    ...
    <script>
        ...
        convertToBase64('fileCardBusiness', 'cardBusiness');
        convertToBase64('fileImageContrato', 'imagecontrato');
        convertToBase64('fileComprovanteEndereco', 'imagecomprovante_endereco');


        document.querySelector('form').addEventListener('submit', function(e) {
            // verifica se todos estão presentes
            const fields = ['imageself', 'imagedoc', 'imagedoc_verso',
                'imagecomprovante', 'cardBusiness',
                'imagecontrato', 'imagecomprovante_endereco'
            ];

            for (const field of fields) {
                if (!document.getElementById(field).value) {
                    alert('Informe todos os documentos antes de prosseguir.');
                    e.preventDefault();
                    return false;
                }
            }
