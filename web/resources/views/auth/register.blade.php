...
<body>
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="verificationModalLabel">Antes de abrir sua conta é necessário
                            validar seu Whatsapp!</h5>
                    </div>
                    <div class="modal-body">
                        <!-- Formulário para enviar o código para WhatsApp -->
                        <form id="whatsappForm" method="POST" action="{{ route('send.phone') }}">
                            @csrf
                            <div class="form-group">
                                <label for="whatsapp">Digite seu número de telefone (WhatsApp):</label>
                            ...
                            <div id="whatsapp-error-message" class="mt-3 text-danger" style="display: none;"></div>
                        </form>

                        <!-- Formulário para validar o código -->
                        <form id="codeForm" method="POST" action="{{ route('validate.code') }}"
                            style="display: none;">
                            @csrf
                            <input type="hidden" name="whatsapp" id="phone-hidden">
                            <div class="form-group">
                                <label for="code">Digite o código recebido:</label>
                                <input type="text" class="form-control" id="code" name="code"
                                    placeholder="Código de 8 dígitos" required>
                            </div>
                            <button type="submit" class="btn btn-success btn-block mt-3">Validar Código</button>
                            <div id="code-error-message" class="mt-3 text-danger" style="display: none;"></div>
                        </form>
                    </div>
                </div>
            </div>
            ...
            <div class="container">
                <div class="mw-md mx-auto text-center">
                    ...
                    <form method="POST" id="documentForm" action="{{ route('register') }}"
                        ...
                        <div class="mb-5">
                            <input class="form-control" type="text" id="cpfCnpj" value="{{ old('document') }}"
                                name="document" placeholder="CPF ou CNPJ" required>
                        </div>
                        <div class="mb-5">
                        ...
                        <div class="mb-5">
                            <select class="form-control" name="gender" required>
                                ...
                                <option value="female" {{ old('gender') === 'female' ? 'selected' : '' }}>Feminino
                                </option>
                                <option value="other" {{ old('gender') === 'other' ? 'selected' : '' }}>Outro
                                </option>
                            </select>
                        </div>
                        <h5>Dados Financeiros</h5>
                        <div class="mb-5">
                            <input class="form-control" type="text" placeholder="Profissão"
                                value="{{ old('profession') }}" name="profession">
                        </div>
                        <div class="mb-5">
                            <input class="form-control" type="text" placeholder="Renda Mensal"
                                value="{{ old('income') }}" name="income" id="income">
                        </div>
                        ...
                        <div class="mb-5">
                            <input class="form-control" type="text" id="cidade" name="city"
                                placeholder="Cidade" value="{{ old('cidade') }}" readonly required>
                        </div>
                        <div class="mb-5">
                        ...
                        <div class="mb-5">
                            ...
                            <div>
                                <!-- Botão para Desktop -->
                                <button type="button" class="btn btn-primary mb-2 desktop-button"
                                    onclick="startWebcam('document_front')">Tire uma Foto da Frente do
                                    Documento</button>
                                ...
                                <button type="button" class="btn btn-secondary mt-2 desktop-button"
                                    onclick="captureImage('document_front')" disabled>Salvar Frente do
                                    Documento</button>

                                <!-- Input de Arquivo para Móveis -->
                                <div class="file-input-container mobile-input">
                                    <label for="frontFile" class="file-input-label">Selecionar Frente do
                                        Documento</label>
                                    <input type="file" accept="image/*" capture="environment" id="frontFile"
                                        onchange="handleFileUpload('document_front', this)">
                        ...
                        <div class="mb-5">
                            ...
                            <div>
                                <!-- Botão para Desktop -->
                                <button type="button" class="btn btn-primary mb-2 desktop-button"
                                    onclick="startWebcam('document_back')">Tire uma Foto do Verso do Documento</button>
                                ...
                                <button type="button" class="btn btn-secondary mt-2 desktop-button"
                                    onclick="captureImage('document_back')" disabled>Salvar Verso do Documento</button>

                                <!-- Input de Arquivo para Móveis -->
                                <div class="file-input-container mobile-input">
                                    <label for="backFile" class="file-input-label">Selecionar Verso do
                                        Documento</label>
                                    <input type="file" accept="image/*" capture="environment" id="backFile"
                                        onchange="handleFileUpload('document_back', this)">
                        ...
                        <div class="mb-5">
                            ...
                            <div>
                                <!-- Botão para Desktop -->
                                <button type="button" class="btn btn-primary mb-2 desktop-button"
                                    onclick="startWebcam('selfie')">Tire uma Selfie com Documento</button>
                                <video id="selfieVideo" autoplay class="hidden"></video>
                                <canvas id="selfieCanvas" class="hidden"></canvas>
                                <div id="selfieImageContainerPreview" class="image-preview"></div>
                                <button type="button" class="btn btn-secondary mt-2 desktop-button"
                                    onclick="captureImage('selfie')" disabled>Salvar Selfie com Documento</button>

                                <!-- Input de Arquivo para Móveis -->
                                <div class="file-input-container mobile-input">
                                    <label for="selfieFile" class="file-input-label">Selecionar Selfie com
                                        Documento</label>
                                    <input type="file" accept="image/*" capture="user" id="selfieFile"
                                        onchange="handleFileUpload('selfie', this)">
    ...
    <script>
        $(document).ready(function() {
            ...
            if (!localStorage.getItem('hasSeenModal')) {
                // Exibe o modal caso seja a primeira vez
            ...
            // Enviar o formulário do WhatsApp
            $('#whatsappForm').on('submit', function(event) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        $('#whatsappForm').hide();
                        $('#phone-hidden').val(response
                        .whatsapp); // Passa o número formatado para o próximo formulário
                        $('#codeForm').show();
                        $('#whatsapp-error-message').hide();
                    },
                    error: function(xhr) {
                        $('#whatsapp-error-message').text(xhr.responseJSON.message).show();
                    }
                });
            });

            // Enviar o formulário do código
            $('#codeForm').on('submit', function(event) {
                ...
                $.ajax({
                    ...
                    error: function(xhr) {
                        $('#code-error-message').text(xhr.responseJSON.message).show();
                    }
                });
            });
        });
    ...
    // Habilitar o botão apenas para teste
    submitButton.disabled = false;

    submitButton.addEventListener("click", function () {
        // Iniciar animação de carregamento
        submitButton.innerHTML = '<div class="spinner"></div>';
        submitButton.classList.add("loading-text");

        // Simular validação (tempo de espera)
        setTimeout(() => {
            // Mostrar ícone de sucesso após a validação
            submitButton.innerHTML = '<span class="success-icon">✔️</span> Conta Criada';
            submitButton.classList.remove("loading-text");

            // Desativar o botão após a criação
            submitButton.disabled = true;
        }, 2000); // 2 segundos
    });
});
