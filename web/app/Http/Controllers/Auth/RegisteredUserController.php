...
class RegisteredUserController extends Controller
{
    ...
    public function create(Request $request, $agency_id)
    {
        ...
        return view('auth.become', ['agency' => $agency->id]);
    }
...
public function store(Request $request)
{
   ...
   try {
    ...
    $folderPath = '/uploads/userdocs/documents/';

    // Salvar imagens base64
    $imageself = saveBase64File($request->imageself, $folderPath, 'imageself');
    $imagedoc = $request->filled('imagedoc') ? saveBase64File($request->imagedoc, $folderPath, 'imagedoc') : null;
    $imagedoc_verso = $request->filled('imagedoc_verso') ? saveBase64File($request->imagedoc_verso, $folderPath, 'imagedoc_verso') : null;
    $imagecomprovante = $request->filled('imagecomprovante') ? saveBase64File($request->imagecomprovante, $folderPath, 'imagecomprovante') : null;
