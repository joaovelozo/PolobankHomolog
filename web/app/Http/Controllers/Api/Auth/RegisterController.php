...
class RegisterController extends Controller
{
    ...
    {
        // Validação dos dados de entrada com mensagens personalizadas
        $validator = Validator::make($request->all(), [
            ...
            'selfie' => 'required|file|mimes:jpeg,png,jpg|max:5120',  // Atualizado para arquivo
            ...
            'plan_id.required' => 'Um plano válido deve ser selecionado.',
        ...
        try {
            ...
            if (!$plan) {
                return response()->json(['message' => 'Plano selecionado é inválido.'], 400);
            }
             // Upload das imagens (documentos e selfie)
            ...
            // Processamento da renda
            $icome = 0.00;
            if (!empty($request->icome)) {
                $icome = $request->icome;
            }
            // Verifica e obtém número da agência
            $agencyNumber = null;
            ...
            $user = User::create([
                ...
                'account' => str_pad(mt_rand(0, 99999999 - 9), 9, '0', STR_PAD_LEFT) . mt_rand(0, 9),
                'agency_id' => $request->input('agency_id'),
                'document_front' => $documentFrontImageName,
                'document_back' => $documentBackImageName,
                'selfie' => $selfieImageName,
            ]);
            ...
            // Processamento de serviços adicionais
            ...
            $incomeFormatted = number_format($icome, 2, ',', '.');
            ...
            // Criação automática de cartão do tipo débito
            $card = new Card();
            ...
            Auth::login($user);
            $token = $user->createToken('PolocalBank')->plainTextToken;
            ...
            // Em caso de erro, faz rollback e registra no log
            DB::rollback();
    ...
    {
        return '1234 5678 3456'; // Exemplo, ajuste conforme a necessidade
    }
